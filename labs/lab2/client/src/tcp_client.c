...
void tcp_client_init(tcp_client *client, const char* server_addr, uint16_t server_port) {
    ...
    if (inet_pton(AF_INET, server_addr, &client->client_addr.sin_addr) != 1) {
    ...
    inet_ntop(AF_INET, &local_addr.sin_addr, client->local_ip, INET_ADDRSTRLEN);
...
void tcp_client_run(tcp_client *client) {
    ...
    while (1) {
        printf("(Клиент %s:%d): ", client->local_ip, client->local_port);
        fgets(message, sizeof(message), stdin);
        message[strcspn(message, "\n")] = '\0';

        if (strcmp(message, "exit") == 0) {
            tcp_client_send_msg(client, message);
            break;
        }

        tcp_client_send_msg(client, message);
        tcp_client_recv_msg(client, response, sizeof(response));
    }
}
